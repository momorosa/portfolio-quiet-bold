...
import { tomoCarePhase0Content as content } from "../contents/tomoCarePhase0.js"
import {
  CaseStudySection,
  CaseStudyBody,
  CaseStudyEyebrow,
...
export default function TomoCarePhase0() {
  return (
    <main className="relative min-h-screen bg-warm-gray-dark/50 text-white font-primary overflow-x-hidden">
      ...
      <article>
        ...
        >
          ...
          <WorkingBrainDiagram className="mt-2" />
...
>
  ...
  {content.principles.items.find((principle) => principle.imgUrl) ? (
    <MediaFigure
      src={content.principles.items.find((principle) => principle.imgUrl).imgUrl}
      caption={content.principles.items.find((principle) => principle.imgUrl).caption}
      alt="TomoCare provenance-first event record evidence"
      className="pt-2"
    />
  ) : null}
